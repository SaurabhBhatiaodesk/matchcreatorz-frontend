...
import { S3_URL, signUpInitialValues, SignUpInterface, signUpSchema } from 'constants/index';
import { IMAGE_PATH } from 'constants/imagePaths';
import ReactSelect from 'react-select';
import { useNavigate } from 'react-router-dom';
import { Loader, AddImage } from 'components';
import { useSignUpMutation, useStateListMutation } from 'services';
...
const RegisterBuyer: React.FC<RegisterBuyerProps> = ({
	userStatus,
	isUploading,
	updateImageData,
	setIsUploading,
	...
	const renderError = <T extends keyof SignUpInterface>(field: T) =>
	...
	const handleCountrySelect = (value: any) => {
		...
		stateListMutation(`?countryId=${value?.id}`)
			.then((res: any) => {
				if (res?.success) {
			...
			<Form className="auth-form" onSubmit={formik.handleSubmit}>
				...
				<Form.Group className="mb-20 ">
					<div className="d-flex position-relative">
						<ReactSelect
							className={`form-react-select phone ${renderError('phone') ? 'border-red' : ''}`}
				...
				<Form.Group className="mb-20" controlId="formBasicEmail">
					<ReactSelect
						...
						value={countryList?.country?.find((item: any) => item.id === formik.values.id)}
						onBlur={formik.handleBlur}
					/>
				...
				<Form.Group className="mb-20" controlId="formBasicEmail">
					<ReactSelect
						...
						getOptionValue={(option: any) => option?.id}
						onChange={(value: any) => formik.setFieldValue('stateId', value?.id)}
						value={countryList?.country?.find((item: any) => item.id === formik.values.id)}
						onBlur={formik.handleBlur}
					/>
					{renderError('stateId')}
				...
				<Form.Group className="mb-20 position-relative" controlId="formBasicPassword">
					...
					/>
					<span className="eye-view" onClick={() => setPasswordVisibleCp(!passwordVisibleCp)}>
