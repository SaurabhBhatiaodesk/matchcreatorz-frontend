...
import { useProfileListMutation, useUpdateStatusBidMutation } from 'services';
import useAuthStore, { setUserInfo } from 'store/auth';
import useCommonStore from 'store/common';
import { setProfileData } from 'store/user';

const OfferRejectAccept: React.FC<any> = () => {
	...
	const { mutateAsync: statusBidUpdateMutation } = useUpdateStatusBidMutation();
	...
	const handleOfferStatusUpdate = () => {
		statusBidUpdateMutation(payload).then((res: any) => {
			if (res?.success) {
				...
				<div className="d-flex flex-column gap-4">
					<div className="d-flex flex-column gap-2 text-center">
						<h2 className="modal-title mb-0 ">
							Bid {payload?.status === 'REJECT' ? 'Reject' : 'Accept'}
						</h2>
						<p className="modal-description mb-0">
							Are you sure you want to {payload?.status === 'REJECT' ? 'reject' : 'accept'} this bid
							?
						</p>
					</div>
					<div className="d-flex gap-2 w-100">
						...
						<button className="secondary-btn mw-100 " onClick={() => handleOfferStatusUpdate()}>
							YES
						</button>
					</div>
				</div>
...
export default OfferRejectAccept;
